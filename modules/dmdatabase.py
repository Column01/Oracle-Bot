...
async def add_dm(userid):
    await asyncio.sleep(0.1)
    if not await existing_dm(userid):
...
async def get_allowed_roles(userid):
    await asyncio.sleep(0.1)
...
async def has_allowed_role(userid, role):
    allowed_roles = await get_allowed_roles(userid)
